...
export default class DEMASRSI extends Indicator {
    MFI!: number;
    AVGSRSI!: number;
    SRSI_D!: number;
    SRSI_K!: number;
    ...
    calculate(k: Klines): Signal {
        ...
        this.DEMA146 = t.dema(k.getCloses(), 146);
        this.PREV_DEMA146 = t.dema(k.getCloses(), 146, -2);
        this.AVGSRSI = (this.SRSI_K + this.SRSI_D) / 2;

        if (this.AVGSRSI < 10 && price < this.DEMA146) {
            return Signal.BUY;
        } else if (price > this.DEMA146 && price < this.PREV_DEMA146 && this.AVGSRSI < 5) {
            return Signal.BUY;
