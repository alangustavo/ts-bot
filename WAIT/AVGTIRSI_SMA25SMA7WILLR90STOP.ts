...
export default class AVGTIRSI_SMA25SMA7WILLR90STOP extends Strategy {
  ...
  public getHeader(): void {
    this.csvFile.addRow(
      `${this.symbol}_${this.interval} BUY: avgTIRSI <= 10 && sma25 > sma7 && rsi < 32 && WILLR > -90| SELL: avgTIRSI >= 90 && sma25 < sma7 | STOPLOSS -0.05`
    );
    this.csvFile.addRow(
  ...
  update(klines: Klines): void {
    ...
    if (!this.inPosition) {
      ...
      if (signal == Signal.SELL || signal === Signal.STOPLOSS) {
