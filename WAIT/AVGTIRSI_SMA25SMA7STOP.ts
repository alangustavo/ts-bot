...
export default class AVGTIRSI_SMA25SMA7STOP extends Strategy {
  ...
  public getHeader(): void {
    this.csvFile.addRow(
      `${this.symbol}_${this.interval} BUY: avgTIRSI <= 10 && sma25 > sma7 && rsi < 32| SELL: avgTIRSI >= 90 && sma25 < sma7 | STOPLOSS -0.05`
    );
    this.csvFile.addRow(
  ...
  update(klines: Klines): void {
    ...
    this.csvFile.addRow(
      `${ot}${this.price};${this.willr};${this.sma99};${this.sma25};${
        this.sma7
      };${this.rsi};${this.avgTIRSI};${this.stochRSID};${this.stochRSIK};${
        this.stochD
      };${this.stochK};${this.adosc};${signal};${pl - 1};${this.result}`
    );
  }
  ...
  public getBuySignal(klines: Klines): Signal {
    ...
    if (this.avgTIRSI <= 10 && this.sma25 > this.sma7 && this.rsi < 32) {
      return Signal.BUY;
    } else {
  ...
  public getSellSignal(klines: Klines): Signal {
    ...
    if (this.price / this.buyPrice - 1 > 0.035) {
      return Signal.STOPGAIN;
    }
