...
class AvgTIRSM99 extends IStrategy_1.Strategy {
    update(klines) {
    ...
    calculate(k) {
        ...
        let sma99 = this.indicator.SMA(k.getCloses(), 99);
        this.sma99 = sma99[sma99.length - 1];
        this.price = closes[closes.length - 1];
        this.avgTIRSI =
            (this.tisrsi.K[i] + this.tisrsi.D[i] + this.tisrsi.stochRSI[i]) / 3;
    }
    getBuySignal(klines) {
        this.calculate(klines);
        if (this.avgTIRSI < 10 && this.price > this.sma99) {
